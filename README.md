...
* **Accessibility & Screen Reader Support:** Centralized screen reader live-region announcer (`aria-live` polite/assertive), accessible modal dialogs (`<dialog>`), high-contrast focus rings, and full keyboard navigation across form controls and quiz cards.
...
1. **Configure Setup:** Choose your target location, optional target taxon, establishment status filter, weighting method (Linear/Logarithmic), media types (photos/sounds), month filters (via manual selection or seasonal presets), pool size, question limit, and duplicate/rare preferences.
...
* `js/api.js`: Low-level wrapper for the iNaturalist v2 API featuring a custom `RequestQueue` throttler, parameter builders, taxa verification, dynamic network timeout calculation, and license file fetching.
...
* `js/ui.js`: Rendering layer mapping state updates directly to DOM nodes using standard DOM methods, screen reader live-region announcements, and hyperlinked TASL media attributions.
* `js/app.js`: Application entry point tying state subscribers, event listeners, user preferences, autocomplete controllers, and modal dialog handlers together.
