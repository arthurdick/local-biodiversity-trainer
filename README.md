...
* **📅 Seeded Daily Challenge Mode:** Take a synchronized 24-hour daily quiz calculated deterministically using a Mulberry32 Pseudo-Random Number Generator (PRNG). Standardizes rules (10 questions, Medium Top 125 pool, current UTC calendar month seasonality, forced free-text mode) so naturalists anywhere on Earth can compete on an even baseline.
* **📊 Emoji Score Cards & Social Sharing:** Export visual score summaries (`🟩🟨🟥⬜`) to your clipboard with a single click to share on social media or chat platforms. Automatically tracks and flags replay practice runs `(Replay)` while recording your initial attempt score for today's daily challenge.
* **🔗 URL Parameter Deep-Linking:** Share custom or daily challenge setups directly via shareable URLs (e.g., `?mode=daily&place=50812`). URL parameters dynamically override runtime state without polluting or overwriting the user's personal `localStorage` preferences.
...
* **Preference Memory & Score Caching:** Automatically remembers location, taxon, user life list settings, media options, seasonality filters, weighting methods, establishment status, quiz difficulty, and multiple choice preferences using `localStorage`. Automatically tracks daily challenge initial scores while purging outdated historical records.
...
1. **Choose Mode:** Select **Daily Mode** for a 10-question standardized daily challenge synchronized by location date seed, or stay in **Custom Mode** to customize your training session.
2. **Configure Setup:** Choose your target location, optional target taxon, optional iNaturalist user life list filter, establishment status filter, weighting method (Linear/Logarithmic), media types (photos/sounds), month filters (via manual selection or seasonal presets), pool size, question limit, and gameplay toggles.
3. **Take the Quiz:** Examine research-grade observation images or listen to recorded audio clips fetched dynamically. Use the full-resolution image zoom or toggle field note hints if needed.
4. **Submit Answers:** In Multiple Choice mode, click or press `1`–`4` to select the species. In Free-Text mode, select your confidence rank (Species, Genus, Family, Order) and type common or scientific names to check your answer against the database.
5. **Share & Review:** Examine your live emoji score card preview upon quiz completion. Click **📊 Share Score Card** or **🔗 Share Link Only** to copy results to your clipboard, and analyze missed species at the end of the session.
...
* `js/state.js`: Single source of truth state store with deep freezing (`Object.freeze`) in development mode, immutable state updates, daily score persistence, and custom event dispatching.
* `js/urlService.js`: Parser and serializer for URL search parameters (`?mode=daily&place=...`), deep linking, secure clipboard interaction, and score card formatting.
* `js/api.js`: Low-level wrapper for the iNaturalist v2 API featuring a custom `RequestQueue` throttler, parameter builders, user autocomplete, taxa verification, similar species lookups, dynamic network timeout calculation, HTTP browser caching for daily challenges, and license file fetching.
* `js/observationService.js`: Prefetching pipeline, connection-aware network timeout calculator, deterministic page offset selector for daily mode, and deep-paging selection manager.
* `js/quizEngine.js`: Core domain algorithms for Mulberry32 PRNG seed key calculations (`YYYY-MM-DD:location`), frequency weighting, deep page calculations, real-world lookalike distractor ranking, exact/alias taxonomic matching, answer evaluation, and hint spoiler redaction.
...
* `js/ui.js`: Rendering layer mapping state updates directly to DOM nodes using standard DOM methods, daily banner logic, locked control inputs, live score card text rendering, screen reader live-region announcements, option skeleton states, and hyperlinked TASL media attributions.
* `js/app.js`: Application entry point tying state subscribers, event listeners, keyboard shortcut handlers (`1`–`4`), URL bootstrapping, daily mode toggles, autocomplete controllers, and modal dialog handlers together.
